...
namespace TeatrLibrary.DataAccess
{
    class SQLConnector : IDataConnection
    {
        ...
        public List<RoleModel> GetRoles()
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnString(db)))
            {
                return connection.Query<RoleModel>($"select * from Role").ToList();
            }
        }

        public int CreatUser(UserModel model)
        {
            DataProtection pass = new DataProtection(model.Password);
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnString(db)))
            {
                DynamicParameters parameter = new DynamicParameters();
                parameter.Add("@name", model.Name);
                ...
                parameter.Add("@mail", model.Mail);
                parameter.Add("@role", model.RoleId);
                parameter.Add("@createdId", 0, dbType: DbType.Int32, direction: ParameterDirection.Output);

                connection.Execute("dbo.spCreateUser", parameter, commandType: CommandType.StoredProcedure);
                model.Id = parameter.Get<int>("@createdId");
                return model.Id;
            }
        }
